import { defineConfig, searchForWorkspaceRoot } from 'vite';
import vue from '@vitejs/plugin-vue';
import legacy from '@vitejs/plugin-legacy';
import { resolve } from 'path';
// import Inspect from 'vite-plugin-inspect'
import VitePluginCertificate from 'vite-plugin-mkcert';
import IconsResolver from 'unplugin-icons/resolver';
import Icons from 'unplugin-icons/vite';
import AutoImport from 'unplugin-auto-import/vite';
import Components from 'unplugin-vue-components/vite';
import { ElementPlusResolver } from 'unplugin-vue-components/resolvers';
// https://cn.vitejs.dev/config/#config-file-resolving
export default defineConfig({
  base: process.env.NODE_ENV === 'production' ? '/biz/' : './', // 设开发或生产环境服务的 公共基础路径
  define: {
    // 类型： Record<string, string> 定义全局变量替换方式。每项在开发时会被定义为全局变量，而在构建时则是静态替换。
    'process.platform': null,
    'process.version': null,
  },
  plugins: [
    // 类型： (Plugin | Plugin[])[]  将要用到的插件数组
    vue(),
    VitePluginCertificate({
      source: 'coding',
    }),
    Icons({
      autoInstall: true,
    }),
    AutoImport({
      resolvers: [ElementPlusResolver()],
      eslintrc: {
        enabled: true, // Default `false`
        filepath: './.eslintrc-auto-import.json', // Default `./.eslintrc-auto-import.json`
        globalsPropValue: true, // Default `true`, (true | false | 'readonly' | 'readable' | 'writable' | 'writeable')
      },
      imports: [
        'vue',
        'vue-router',
        'vuex',
        {
          '@/utils/buttonAuth': ['AuthConsumer'],
          '@/utils/helper': ['formatMoney'],
        },
      ],
      dts: resolve('src/auto-imports.d.ts'),
    }),
    Components({
      extensions: ['vue', 'tsx'],
      resolvers: [
        // Auto register icon components
        // 自动注册图标组件
        IconsResolver({
          enabledCollections: ['ep'],
        }),
        ElementPlusResolver(),
      ],
      dts: resolve('src/components.d.ts'),
    }),
    legacy({
      targets: ['defaults', 'not IE 11'],
    }),
    // Inspect()
  ],
  resolve: {
    // 类型：Record<string, string> | Array<{ find: string | RegExp, replacement: string }> 将会被传递到 @rollup/plugin-alias 作为它的 entries。
    alias: {
      '~': resolve(__dirname, './'),
      '@': resolve(__dirname, 'src'),
    },
    extensions: ['.js', '.ts', '.jsx', '.tsx', '.json', '.vue', '.mjs'], // 类型： string[] 导入时想要省略的扩展名列表。
  },
  clearScreen: false,
  server: {
    https: true,
    hmr: { overlay: false }, // 禁用或配置 HMR 连接 设置 server.hmr.overlay 为 false 可以禁用服务器错误遮罩层
    // define WorkSpaceRoot to resolve Warn, please visit https://vitejs.dev/config/#server-fs-allow
    fs: {
      allow: [searchForWorkspaceRoot(process.cwd())],
    },
    //response headers
    headers: {
      "Cross-Origin-Opener-Policy": "same-origin",
      "Cross-Origin-Embedder-Policy": "require-corp",
    },
    // 服务配置
    port: 80, // 类型： number 指定服务器端口;
    open: true, // 类型： boolean | string在服务器启动时自动在浏览器中打开应用程序；
    cors: true, // 类型： boolean | CorsOptions 为开发服务器配置 CORS。默认启用并允许任何源
    proxy: {
      // 类型： Record<string, string | ProxyOp 为开发服务器配置自定义代理规则
      '/api': {
        // 线上联调地址
        target: 'http://172.1.2.181:9999/', // 测试环境
        // 改成你跟后端的本地联调地址
        // target: 'http://172.16.17.89:9999/', //陈瑞
        // target: 'http://172.16.17.51:9999/', //邵一帆

        changeOrigin: true,
        secure: false,
        rewrite: (path) => path.replace('/api', ''),
      },
    },
  },
  // https://www.vitejs.net/config/#build-commonjsoptions
  // build: {
  //   brotliSize: false, // 禁用会提高大型项目的构建性能。
  //   commonjsOptions: {
  //     ignoreDynamicRequires: false, // Default: false
  //     transformMixedEsModules: true,
  //     sourceMap: false
  //   }
  // },
  // optimizeDeps: {
  //   include: ['element-plus/lib/locale/lang/zh-cn', 'element-plus/lib/locale/lang/en']
  // }
});
